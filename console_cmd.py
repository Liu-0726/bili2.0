...
class ConsoleCmd(Cmd):
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, room_id, printer_danmu):
        ...
        self._parser_21 = self.compile_parser('21 || --user_id/-u {int?0} || --room_id/-p {room_id?%default_roomid}'
                                              ' || --num/-n {int}')
        self._parser_22 = self.compile_parser('22 || --user_id/-u {int?0} || --room_id/-p {room_id?%default_roomid}'
                                              ' || --num/-n {int}')
    ...
    def do_21(self, arg):
        user_id, real_roomid, num_max = self.parse(arg, self._parser_21)
        self.exec_func_threads(
            FuncCore(notifier.exec_task, SendLatiaoTask, 'cmd', user_id, real_roomid, num_max))

    def do_22(self, arg):
        user_id, real_roomid, num_wanted = self.parse(arg, self._parser_22)
        self.exec_func_threads(
